...
import { getAuth, signInWithPopup, GoogleAuthProvider } from "firebase/auth";  // from firebase doc
    ...
    <div className='hidden md:flex w-full h-full items-center justify-between'>
        ...
        <div onClick={showCart} className='relative flex items-center justify-center'>
         ...
         {isMenu && (
          ...
          {user && user.email === "kunal770609@gmail.com" && (
            <Link to={"/createItem"}>
            <p className='px-4 py-2 flex items-center gap-3 cursor-pointer hover:bg-slate-100 transition-all duration-100 ease-in-out test-testColor test-base' onClick={()=> setIsMenu(false)}>New Item <MdAdd/></p>
            </Link>
          )}
          <p className='px-4 py-2 flex items-center gap-3 cursor-pointer hover:bg-slate-100 transition-all duration-100 ease-in-out test-testColor test-base'
      ...
      <div className='flex items-center justify-between md:hidden w-full h-full'>
        ...
        <div className='relative'>
         ...
         {isMenu && (
          ...
          {user && user.email === "kunal770609@gmail.com" && (
            <Link to={"/createItem"}>
            <p className='px-4 py-2 flex items-center gap-3 cursor-pointer hover:bg-slate-100 transition-all duration-100 ease-in-out test-testColor test-base'>New Item <MdAdd/></p>
            </Link>
          )}
          ...
          <p className='m-2 p-2 rounded-md shadow-md flex items-center justify-center bg-gray-200 gap-3 cursor-pointer hover:bg-gray-300 transition-all duration-100 ease-in-out test-testColor test-base' onClick={logOut}>Logout <MdLogout/></p>
         </motion.div>
          )
         }
         </div>
